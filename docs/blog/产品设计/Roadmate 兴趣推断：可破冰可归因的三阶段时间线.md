---
title: 产品设计：Roadmate 兴趣推断——可破冰、可归因的三阶段时间线
tags:
  - Roadmate
  - LLM
  - 产品设计
...
线下见面最怕的是空泛标签。「音乐 / 旅行」听起来人人都有，却开不了口。Roadmate 需要的是「周末 pour-over」「嵌入式 Rust」这类见面就能聊的具体话题，并且能说清：这个兴趣出现得多频繁、有多新、靠哪几条帖子撑起来。

本文整理 [Roadmate](https://github.com/1nFrastr/roadmate) 兴趣推断流水线：三阶段时间线抽出标签，再用代码算权重与 embedding，驱动词云和近场匹配分。
...
设备交互见 [近场设备 Playground 设计](/article/5bb8d297/)。
...
断言可检查关键词命中、禁词、标签数量、有效帖下限。`--verbose` 会打印每帖噪声判断、合并条目、最终权重表，便于定位是哪一阶段出了问题。
