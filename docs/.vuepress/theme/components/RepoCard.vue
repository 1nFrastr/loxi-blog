...
<template>
  <div v-if="data?.name" class="vp-repo-card" v-bind="$attrs">
    <p class="repo-name">
      ...
      <span class="repo-link">
        <a
          ...
          :title="data.fullName"
        >
          {{ showFullname ? data.fullName : data.name }}
        </a>
      </span>
      <span class="repo-visibility" :class="{ archived: data.archived }">
  ...
  <div v-else class="vp-repo-card vp-repo-card--fallback" v-bind="$attrs">
    <p class="repo-name">
      <span :class="`vpi-${provider}-repo`" />
      <span class="repo-link">
        <a
